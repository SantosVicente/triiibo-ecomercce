# triiibo-ecomercce

## Breve descrição

triiibo-ecomercce é um repositório para o projeto de um e-commerce de Engenharia de Software com foco em artigos de acampamento. O objetivo é documentar requisitos, UX/UI, fluxos de negócio, testes e um backlog priorizado para implementação incremental (sprints).

## Arquivos e pastas (atuais e previstas)

- [README.md](README.md)
- [METRICS.md](METRICS.md) — métricas de estimativa, progresso, qualidade e escopo.
- [PROPOSTA-DE-CONFIGURACAO-DE-SISTEMA.md](PROPOSTA-DE-CONFIGURACAO-DE-SISTEMA.md) — decisões técnicas, stack, infraestrutura e CI/CD.
- `docs/` — documentação complementar, requisitos, narrativas e plano de testes.
- `design/` — diagramas (UML, casos de uso), wireframes e arquivos do Figma/Draw.io.

## Links

> Todos os acessos foram concedidos aos membros da equipe e ao professor no email: anderson.barbosa@cps.sp.gov.br
> Qualquer dúvida ou necessidade de acesso, por favor, entre em contato.

- [Jira](https://vicentedossantos.atlassian.net/?continue=https%3A%2F%2Fvicentedossantos.atlassian.net%2Fwelcome%2Fsoftware%3FprojectId%3D10000&atlOrigin=eyJpIjoiZWJmNmFhYjAzMTBlNGNmOWEzNTZkYmNiNjcyYTc5NGIiLCJwIjoiamlyYS1zb2Z0d2FyZSJ9)
- [Figma](https://www.figma.com/design/VyLzHtJWcumPCNogXmenlr/Triiibo---Design?node-id=0-1&t=gW2blgxhf4oXjQkn-1)
- [Figma Protótipo](https://www.figma.com/proto/VyLzHtJWcumPCNogXmenlr/Triiibo---Design?node-id=33-1079&p=f&t=f78iASD8R5rnfoDu-1&scaling=min-zoom&content-scaling=fixed&page-id=0%3A1&starting-point-node-id=33%3A1079)

## Contato

Equipe/Responsável:

- Nome: Ana Beatriz Mendes São Pedro Digrocco - ana.digrocco@aluno.cps.sp.gov.br
- Nome: Eros de Lacerda Quadros - eros.quadros@aluno.cps.sp.gov.br
- Nome: Paulo Eduardo Ferreira Junior - paulo.ferreira4@aluno.cps.sp.gov.br
- Nome: Vicente dos Santos Silva - vicente.silva2@aluno.cps.sp.gov.br
- Nome: Vicente Matheus Collin Pedroso - vicente.pedroso@aluno.cps.sp.gov.br
